...
import { getSiteUrl } from "../_shared/talk-review-email.ts";
import { sendTalkEventAssignmentEmail } from "../_shared/talk-event-assignment-email.ts";
...
interface TalkAssignmentNotification {
...
async function getTalkAssignmentNotifications(
  talkIds: string[],
): Promise<TalkAssignmentNotification[]> {
  return sql<TalkAssignmentNotification[]>`
    select
      submission.speaker_email,
      submission.speaker_first_name,
    ...
    where talk."id" in ${sql(talkIds)}
    order by talk."sort_order", talk."title"
  `;
}
...
async function removeUploadedImage(imagePath: string | null): Promise<void> {
  ...
  if (error) {
    console.error("create-event image cleanup failed", error.message);
  }
}
...
Deno.serve(async (req) => {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY || !supabaseAdmin || !DATABASE_URL) {
    return jsonResponse(
      500,
      { error: "create_event_not_configured" },
      corsHeaders,
    );
  }
  ...
  try {
    ...
    if (!(featureGraphic instanceof File)) {
      return jsonResponse(
        400,
        { error: "event_feature_graphic_required" },
        corsHeaders,
      );
    }
    ...
    const { data, error } = await supabaseUser.rpc("create_event_with_talks", {
      p_title: title,
      p_starts_at: startsAt.toISOString(),
      p_meetup_url: meetupUrl,
      p_venue_id: venueId,
      p_feature_graphic_url: publicUrlData.publicUrl,
      p_public: isPublic,
      p_talk_ids: talkIds,
    });
    ...
    const event = (data as CreateEventResult[] | null)?.[0] ?? null;
    ...
    if (!event) {
      ...
      return jsonResponse(
        500,
        { error: "create_event_result_missing" },
        corsHeaders,
      );
    }
    ...
    try {
      const notifications = await getTalkAssignmentNotifications(talkIds);
      const notificationResults = await Promise.allSettled(
        notifications.map((notification) =>
          sendTalkEventAssignmentEmail({
            eventSlug: event.slug,
            eventStartsAt: startsAt.toISOString(),
            eventTitle: title,
            meetupUrl,
            siteUrl: getSiteUrl(),
            speakerEmail: notification.speaker_email,
            speakerFirstName: notification.speaker_first_name,
            speakerName: notification.speaker_name,
            talkTitle: notification.talk_title,
          })
        ),
      );
      ...
      for (const notificationResult of notificationResults) {
        if (notificationResult.status === "rejected") {
          assignmentEmailResults.failed += 1;
          console.error(
            "talk-event-assignment-email failed",
            notificationResult.reason,
          );
        } else if (notificationResult.value) {
          assignmentEmailResults.notified += 1;
        } else {
          assignmentEmailResults.skipped += 1;
        }
      }
    } catch (error) {
      assignmentEmailResults.failed += talkIds.length;
      console.error("talk-event-assignment-email lookup failed", error);
    }
    ...
    return jsonResponse(
      ...
      {
        ...event,
        feature_graphic: publicUrlData.publicUrl,
        assignment_email_results: assignmentEmailResults,
      },
      corsHeaders,
    );
  } catch (error) {
    ...
    if (error instanceof Error) {
      console.error("create-event failed", error.message, error.stack);
    } else {
