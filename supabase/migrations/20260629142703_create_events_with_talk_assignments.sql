...
create or replace function private.slugify_event_title(p_title text)
returns text
language sql
immutable
strict
  ...
  select coalesce(
    nullif(
      trim(
        both '-'
        from regexp_replace(
          lower(regexp_replace(btrim(p_title), '^Angular Zurich[[:space:]]+', '', 'i')),
          '[^a-z0-9]+',
          '-',
          'g'
        )
...
create or replace function public.can_current_user_manage_events()
returns boolean
language sql
stable
security invoker
...
revoke execute on function public.can_current_user_manage_events() from public, anon;
grant execute on function public.can_current_user_manage_events() to authenticated, service_role;
...
revoke execute on function private.create_event_with_talks(
  text,
  timestamptz,
  text,
  uuid,
...
grant execute on function private.create_event_with_talks(
  text,
  timestamptz,
  text,
  uuid,
