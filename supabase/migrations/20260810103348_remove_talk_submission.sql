...
begin
  ...
  delete from submissions.talk_submissions
...
grant execute on function private.remove_talk_submission(uuid) to authenticated, service_role;
revoke execute on function private.remove_talk_submission(uuid) from anon, public;

create or replace function public.remove_talk_submission(
  p_submission_id uuid
)
returns table (
  id uuid,
...
as $$
  select *
  from private.remove_talk_submission(p_submission_id);
$$;

grant execute on function public.remove_talk_submission(uuid) to authenticated;
revoke execute on function public.remove_talk_submission(uuid) from anon, public;
